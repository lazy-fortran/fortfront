...
- **Robust Error Handling**: Graceful fallback for invalid syntax with enhanced error reporting
...
- **Control Flow Support**: Handles if-then-else, do loops, select case, and nested structures
...
fortfront validates input and provides enhanced error reporting before transformation.

**✅ Accepted Input Types:**
- Comments and empty input
- Variable assignments (`x = 42`)
- Mathematical expressions (`a + b`, `(x + y) * z`)
- Function calls (`sqrt(x)`, `sin(angle)`)
- Array operations (`arr(i)`, `matrix(i, j)`)
- Control structures (`if-then-end if`, `do-end do`)
- Nested control structures (loops within conditionals, etc.)
