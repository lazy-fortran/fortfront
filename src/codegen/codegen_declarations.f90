module codegen_declarations
   ...
   use codegen_utilities, only: parameter_info_t, int_to_string, &
                                ...
                                generate_grouped_body_context, find_parameter_info, result_info_t, type_name_from_mono
...
contains
   ...
   function generate_code_function_def(arena, node, node_index) result(code)
      ...
      block
         ...
         type(result_info_t) :: result_info
         ...
         do i = 1, param_count
            ...
            param_map(i)%has_explicit_declaration = .false.
            ...
            if (node%param_indices(i) > 0 .and. &
               ...
               if (allocated(arena%entries(node%param_indices(i))%node)) then
                  ...
                  type is (identifier_node)
                     ...
                     inferred_type_name = ''
                     if (param_node%inferred_type%kind > 0) then
                        if (len_trim(param_map(i)%type_name) == 0) then
                           inferred_type_name = type_name_from_mono(param_node%inferred_type)
                           if (len_trim(inferred_type_name) > 0) then
                              param_map(i)%type_name = trim(inferred_type_name)
                           end if
                        end if
                     end if
                  type is (parameter_declaration_node)
                     ...
                     type is (parameter_declaration_node)
                        ...
                        do i = 1, param_count
                           if (allocated(param_map(i)%name) .and. &
                              ...
                              if (len_trim(param_map(i)%type_name) == 0) then
                                 if (len_trim(body_node%type_name) > 0) then
                                    param_map(i)%type_name = trim(body_node%type_name)
                                 end if
                              end if
                           end if
                        end do
                        ...
                        do i = 1, param_count
                           if (allocated(param_map(i)%name) .and. &
                              ...
                              if (len_trim(param_map(i)%type_name) == 0) then
                                 if (len_trim(body_node%type_name) > 0) then
                                    param_map(i)%type_name = trim(body_node%type_name)
                                 else if (body_node%inferred_type%kind > 0) then
                                    inferred_type_name = type_name_from_mono(body_node%inferred_type)
                                    if (len_trim(inferred_type_name) > 0) then
                                       param_map(i)%type_name = trim(inferred_type_name)
                                    end if
                                 end if
         ...
         ! Determine result variable information for declaration synthesis
         if (allocated(node%result_variable)) then
            if (len_trim(node%result_variable) > 0) then
               result_info%name = trim(node%result_variable)
            end if
         end if
         if (.not. allocated(result_info%name)) then
            if (len_trim(node%name) > 0) then
               result_info%name = trim(node%name)
            end if
         end if
         if (allocated(node%return_type)) then
            if (len_trim(node%return_type) > 0) then
               result_info%type_name = trim(node%return_type)
            end if
         end if
         if ((.not. allocated(result_info%type_name) .or. len_trim(result_info%type_name) == 0) .and. &
             node%inferred_type%kind > 0) then
            inferred_type_name = type_name_from_mono(node%inferred_type)
            if (len_trim(inferred_type_name) > 0) then
               result_info%type_name = trim(inferred_type_name)
            end if
         end if
         ...
         if (allocated(node%body_indices)) then
            code = code//generate_grouped_body_with_params(arena, &
                                                           node%body_indices, 1, param_map, node, result_info)
         end if
      end block
   ...
   contains
      ...
      function mono_type_to_string(mono) result(type_name)
         ...
         case (TCHAR)
            if (mono%size > 0) then
               write (size_buf, '(I0)') mono%size
               type_name = 'character(len='//trim(size_buf)//')'
            else
               type_name = 'character(len=:), allocatable'
            end if
         case default
            type_name = ''
         end select
