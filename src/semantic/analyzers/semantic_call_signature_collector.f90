...
contains
    ...
    subroutine collect_call_signature(signatures, arena, call_node, return_type, &
        ...
        if (allocated(call_node%arg_indices)) then
            n_args = size(call_node%arg_indices)
        else
            n_args = 0
        end if
        ...
        do i = 1, n_args
            arg_type = get_inferred_type_from_arena_local(arena, &
        ...
        if (deduced_kind <= 0) then
            deduced_kind = return_type_copy%kind
        end if
        ...
        if (deduced_kind <= 0) then
            deduced_kind = type_string_to_kind(return_type_tmp)
        end if
