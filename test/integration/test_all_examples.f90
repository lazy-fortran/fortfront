...
contains
    ...
    subroutine process_skip_line(raw_line, count_only, counter, storage)
        ...
        cleaned = adjustl(raw_line)
        call strip_control_characters(cleaned)
        if (len_trim(cleaned) == 0) return
        if (cleaned(1:1) == '#') return

        hash_pos = index(cleaned, '#')
        if (hash_pos > 0) then
            cleaned = adjustl(cleaned(1:hash_pos - 1))
        end if
        cleaned = trim(cleaned)
        if (len_trim(cleaned) == 0) return

        counter = counter + 1
        if (.not. count_only .and. present(storage)) then
            storage(counter) = cleaned
        end if
    end subroutine process_skip_line
