program test_json_modules
    ! Re-enabled after build system fix
...
contains
    ...
    function test_token_json_write() result(passed)
        ...
        ! Write to JSON - re-enabled
        call json_write_tokens_to_file(tokens, filename)
        print *, '  Called json_write_tokens_to_file'
        ...
        if (.not. passed) then
            ...
            print *, '  PASSED: JSON file created successfully'
        end if
        
        ! Clean up
        call delete_file_portable(filename)
        
        print *, '  Token JSON write test completed'
    end function
