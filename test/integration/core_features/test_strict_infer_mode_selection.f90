program test_strict_infer_mode_selection
    ...
    call read_example('examples/lf/issue_2588_bare_assignment.lf', source)
    ...
    ctx%operating_mode = OPERATING_MODE_STRICT
    call transform_with_context(source, output, error_msg, ctx)
    call assert_contains(error_msg, 'Undefined variable')

    print *, 'PASS: strict/infer mode selection enforced'
...
end program test_strict_infer_mode_selection
