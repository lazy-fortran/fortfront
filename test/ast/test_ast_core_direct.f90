program test_ast_core_direct
    ...
    print *, "=== AST Core Direct Function Tests ==="
    print *, ""
    ...
    if (arena%size == 0 .and. arena%capacity >= 256) then
        ...
        print *, "  Got: size=", arena%size, ", capacity=", arena%capacity
    end if
    ...
    if (arena%size == 1 .and. arena%current_index == 1) then
        ...
        call test_fail()
        print *, "  Expected: size=1, current_index=1"
        print *, "  Got: size=", arena%size, ", current_index=", &
                 arena%current_index
    end if
    ...
    if (arena%size == 2 .and. arena%current_index == 2) then
        ...
        call test_fail()
        print *, "  Expected: size=2, current_index=2"
        print *, "  Got: size=", arena%size, ", current_index=", &
                 arena%current_index
    end if
    ...
    if (arena%size == 0 .and. arena%current_index == 0 .and. &
        ...
        call test_fail()
        print *, "  Expected: size=0, current_index=0, max_depth=0"
        print *, "  Got: size=", arena%size, ", current_index=", &
                 arena%current_index, ", max_depth=", arena%max_depth
    end if
    ...
    if (passed_tests == total_tests) then
        print *, "All AST core direct tests passed!"
        stop 0
    else
        print *, "Some AST core tests failed!"
        stop 1
    end if
