...
To enable package managers to implement cross-module specialization, fortfront provides:

**1. Type Signature Export (JSON/TOML metadata)**
```json
{
  "module": "m_add",
  "functions": [
    {
      ...
      "generic": true,
      "specializations": [
        {"signature": "i32_i32", "mangled_name": "add__i32_i32"},
        {"signature": "r64_r64", "mangled_name": "add__r64_r64"}
      ]
    }
  ]
}
...
```json
{
  ...
  "calls": [
    ...
    },
    {
      "function": "add",
      "location": "line 8",
      "argument_types": ["c64", "c64"]
    }
  ]
}
```

**3. Instantiation Template Generation**

Fortfront can generate instantiation module template:
```fortran
! Generated by fortfront for package manager
module m_add_inst
  use m_add, only: add
  implicit none
  private
  ...
  interface add
    module procedure add__c64_c64
  end interface
contains
  complex(8) function add__c64_c64(a, b)
...
fortfront --instantiate module=m_add,sig=c64_c64 -o inst.f90

# Query what specializations exist in a module
fortfront --query-module m_add.mod
```
...
6. Package manager compares caller needs vs library provides
7. If gap exists, package manager invokes: `fortfront --instantiate ...`
8. Package manager orchestrates build: `lib.o` + `inst.o` + `caller.o` → `exe`
9. All artifacts cached globally by package manager
...
2. Add `--instantiate` command to generate instantiation templates
3. Design metadata format (JSON/TOML) for package manager consumption
4. Add `--query-module` to inspect compiled module interfaces
5. **Goal:** Enable package managers to orchestrate cross-module specialization
...
- **Effort:** Low (mostly export existing data)
- **Risk:** Low (package managers consume, fortfront just provides)
